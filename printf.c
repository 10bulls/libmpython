...
//            while (*f != '\0' && *f != '%') {
		...
		if (*fmt == '\n') {
			...
			{
				// send \r\n
				pfenv->print_strn(pfenv->data, _EOL, sizeof(_EOL));
				chrs += sizeof(_EOL);
			}
			// move past \n
			fmt++;
...
// static const 
static pfenv_t pfenv_stdout = {0, stdout_print_strn};

void * set_stdout_callback(void (*fn)(void *, const char *, unsigned int ))
{
	void * f_old = (void*)pfenv_stdout.print_strn;
	pfenv_stdout.print_strn = fn;
	return f_old;
}
...
//!    stdout_print_strn(0, &chr, 1);
	pfenv_stdout.print_strn(0, &chr, 1);
    return chr;
}
...
//!    stdout_print_strn(0, s, strlen(s));
	pfenv_stdout.print_strn(0, s, strlen(s));
//!    char chr = '\n';
//!    stdout_print_strn(0, &chr, 1);
	
	pfenv_stdout.print_strn(0, _EOL, sizeof(_EOL));
    return 1;
}
